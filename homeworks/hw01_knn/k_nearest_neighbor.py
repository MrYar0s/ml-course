...
class KNearestNeighbor:
    ...
    def compute_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        dists = np.sqrt(np.sum(np.power(self.X_train,2),axis=1)-2*np.dot(X,self.X_train.T)+np.array([np.sum(np.power(X,2),axis=1)]).T)
        #dists = np.sqrt(np.sum(np.power(X[:,None] - self.X_train,2),axis=-1))
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
